'''
说明:
dataframe 每一行都是一个dict, 每一列都是一个list
'''
import numpy as np
import pandas as pd
...
def test_1():
    ...
    assert [0] == df1.columns.values.tolist() and [
        0, 1
    ] == df1.index.values.tolist(), 'df error'
    # 为什么值会是这个呢, 因为dataframe是一个二维数组,所以会自动将1维广播成2维
    assert [['x'], ['y']] == df1.values.tolist(), 'df 2 error'

    # 2.指定列名
    df2 = pd.DataFrame(['x', 'y'], columns=['第一列'])
    assert ['第一列'] == df2.columns.values.tolist() and [
        0, 1
    ] == df2.index.values.tolist(), 'df 3 error'
    ...
    assert [[1, 2, 0, 0], [0, 0, 3, 4]] == df4.values.tolist(), 'df 8 error'
...
def test_2():
    ...
    assert pd.Series == type(sr1) and [1, 2] == sr1.values.tolist() and [
        'c1', 'c2'
    ] == sr1.index.values.tolist(), 'df loc error'

    sr2 = df1.loc[[0, 1]]
    assert pd.DataFrame == type(sr2) and [[1, 2], [
        3, 4
    ]] == sr2.values.tolist() and [0, 1] == sr2.index.values.tolist() and [
        'c1', 'c2'
    ] == sr2.columns.values.tolist(), 'df loc 2 error'
...
def test_3():
    ...
    assert pd.Series == type(sr1) and [0, 1] == sr1.index.values.tolist() and [
        1, 4
    ] == sr1.values.tolist(), 'df col error'

    sr2 = df1[['c1', 'c2']]
    assert pd.DataFrame == type(sr2) and [
        0, 1
    ] == sr2.index.values.tolist() and [[1, 2], [
        4, 5
    ]] == sr2.values.tolist(), 'df col error'
